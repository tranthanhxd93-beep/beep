...
// ==== Tách InputField ra ngoài để giữ reference ====
const InputField = React.memo(({ label, value, onChange, editable = true, extraStyle = {}, multiline = false, keyboardType }) => (
  <>
...
export default function ModalThemChuong_P2({ isOpen, onClose, cages=[], editingCage=null, parentId=null }) {
  // ===== State từng field riêng =====
  ...
  // ==== Tính ngày sinh và tách con dự báo ====
  useEffect(()=>{
  ...
  const handleSave=async()=>{
    if(!type) return alert("Chọn loại dúi!");
    const data = {
  ...
  return (
    <ScrollView contentContainerStyle={{paddingBottom:20}}>
      ...
      {/* Date picker không wrap TextInput */}
      {(type==="Cái"||type==="Đực") && <>
        <Text style={{fontWeight:"600",marginBottom:4}}>Ngày phối</Text>
        <TouchableOpacity onPress={()=>openPicker("mating")} style={{backgroundColor:"#e5e7eb",padding:10,borderRadius:6,marginBottom:12}}>
          <Text>{matingDate ? new Date(matingDate).toLocaleDateString() : "Chọn ngày"}</Text>
        </TouchableOpacity>

        <Text style={{fontWeight:"600",marginBottom:4}}>Ngày tách phối</Text>
        <TouchableOpacity onPress={()=>openPicker("separate")} style={{backgroundColor:"#e5e7eb",padding:10,borderRadius:6,marginBottom:12}}>
          <Text>{separateDate ? new Date(separateDate).toLocaleDateString() : "Chọn ngày"}</Text>
        </TouchableOpacity>
      </>}
